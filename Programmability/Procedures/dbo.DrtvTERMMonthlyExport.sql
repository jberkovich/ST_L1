...
GO
 CREATE procedure [dbo].[DrtvTERMMonthlyExport] 
 as
...
Declare @toDay  as datetime 
 ...
 (select dateForExportWeekly from ST_L1.dbo.DatesForExport) ,Dateadd(m,Case When getDate()<'20181001' then 0 else -1 end,Dateadd(d,1 -datepart(d,@toDay),@toDay)))  

 
 set @dateTo= DATEADD(mm,1,@datefrom)
...
from (Select 
 m.*   from main m where spn  in(select spn From DRTVTERMNumber) 
